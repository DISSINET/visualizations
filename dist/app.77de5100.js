...
var loadData = function loadData(next) {
  fetch('https://spreadsheets.google.com/feeds/list/1sCrFQpaesWHWXZcrAV34VmGETXxB4rKYGFw8nk1cmh0/1/public/values?alt=json').then(function (response) {
    return response.json();
  }).then(function (json) {
    var records = [];
    json.feed.entry.map(function (entry) {
      var record = {};
      Object.keys(entry).forEach(function (key) {
        if (key.indexOf('gsx$') > -1) {
          var keyName = key.replace('gsx$', '');
...
var drawParallels = function drawParallels(svg, subsets) {
  loadData(function (records) {
    ...
    */
    ...
    var scales = {
      linear: function linear(scalePar) {
    ...
    */
    ...
    data.map(function (record, ri) {
      record.posM = Object.keys(record.values).map(function (key, vi) {
        var value = record.values[key];
        var par = pars.find(function (par) {
          return par.attr === key;
        });
